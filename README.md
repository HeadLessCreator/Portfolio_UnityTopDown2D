...
> **Tip:** 여기에 게임 플레이 화면, 팝업 애니메이션, 게임 오버 전환 등의 핵심 흐름을 보여주는 GIF나 이미지를 배치해 주세요. 
> 
> * `[게임 플레이 및 적 스폰 GIF]`
> * `[UI 상태 전환 및 팝업 호출 GIF]`
...
코드 퀄리티와 타협하지 않고, 유지보수가 용이한 독립 컴포넌트 구조(MSA 형태)를 지향하며 아래와 같은 설계적 결정을 내렸습니다.
...
현재 구조에서 머무르지 않고, 더 나은 아키텍처로 도약하기 위해 아래와 같은 개선을 계획하고 있습니다.

* **의존성 주입(DI) 기반 결합도 축소:** 현재의 Singleton 기반 접근 방식을 개선하여, 향후 VContainer 또는 Zenject와 같은 인터페이스 기반 의존성 주입 프레임워크를 도입해 유닛 테스트가 용이한 아키텍처로 고도화할 예정입니다.
* **Addressables 참조 해제 및 메모리 정책 강화:** 현재의 LRU 기반 캐시 정책에서 더 나아가, 에셋 생명주기 및 씬 전환에 맞춘 엄격한 `Release()` 호출 루틴을 프로파일링하여 메모리 누수를 원천 차단할 계획입니다.
* **데이터 주도 설계(DDD) 확장:** 하드코딩된 설정이나 기본값 중심의 `ConfigManager`를 개선하여, 향후 ScriptableObject 및 JSON 기반의 원격(Remote) 설정 로딩 파이프라인으로 확장할 것입니다.
* **입력 시스템 고도화:** PC와 모바일 입력(Virtual Joystick) 정책을 인터페이스 레벨에서 더 명확히 분리하여 크로스 플랫폼 대응을 유연하게 개선하겠습니다.
